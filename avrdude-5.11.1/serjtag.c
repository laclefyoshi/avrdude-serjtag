...
//#define USE_WRITE_PAGE_EEPROM
#define USE_INLINE_WRITE_PAGE_EEPROM
...
static int serjtag_set_delay(PROGRAMMER * pgm, double bitclock) {
   use_delay = JTAG_USE_DELAY;
  if (bitclock == 0.0) { // using default
	delay_param = 3;
  } else if (bitclock >= 4.0) {
	delay_param = 0;
	use_delay = 0;
  } else {
  	delay_param =  (int)((2.26/3.0) / bitclock);
  	if (delay_param > 15) delay_param = 15;
  }
  if ((verbose>=1) || SERJTAG_DEBUG) {
	fprintf(stderr," serjtag:delay %d (%s) ( bitclk %.3f )\n"
		,delay_param, use_delay? "enabled":"disabled", bitclock);
  }
  return 0;
}
...
static int serjtag_program_enable(PROGRAMMER * pgm, AVRPART * p)
{
  ...
  int retry_count = 0;
   ...
   serjtag_set_delay(pgm, pgm->bitclock);
...
{
  /*
   *  If baudrate was not specified use 115.200 Baud
   */
  if(pgm->baudrate == 0) {
    pgm->baudrate = 115200;
  }
...
static void serjtag_flush(PROGRAMMER * pgm) {
   ...
   serjtag_send(pgm, (char *)serjtag_buf, serjtag_buf_len);
   serjtag_buf_len = 0;
}
...
{
  unsigned int    i,j;
  int addr,addr_save,addr_wk,buf_pos,do_page_write;
  unsigned int mask;
  char buf[128];
  ...
  for (i=0; i<n_bytes; ) {
	...
	if (m->op[AVR_OP_LOAD_EXT_ADDR]) {
	    ...
	    avr_set_bits(lext, (unsigned char *)buf+buf_pos);
	    avr_set_addr(lext, (unsigned char *)buf+buf_pos, addr/2);
            buf_pos += 4;
	}
        buf[buf_pos++] = 'd';               /* PUT TDI Stream */
        buf[buf_pos++] = JTAG_RECIEVE | use_delay; 
...
#endif
     serjtag_write(pgm, (unsigned char *)buf, buf_pos);
#if 0
...
#ifdef USE_WRITE_PAGE_EEPROM
static int serjtag_paged_write_eeprom(PROGRAMMER * pgm, AVRPART * p, AVRMEM * m, 
                                    int page_size, int n_bytes)
{
  unsigned int    i,j;
  int addr,addr_save,addr_wk,buf_pos,do_page_write;
  ...
  for (i=0; i<n_bytes; ) {
     ...
     for (j=0; j<(JTAG_BUFSIZE-3)/4; j++) { // 15 bytes
        buf[buf_pos++] = 0xc1;
        buf[buf_pos++] = ((addr & mask) >> 8) & 0xff;
        buf[buf_pos++] =  (addr & mask) & 0xff;
        buf[buf_pos++] = m->buf[i];
	addr ++;
	i++;
	if (((i % m->page_size) == 0) || (i == n_bytes)) {
     ...
     buf[2] = buf_pos - 3;
#ifdef USE_INLINE_WRITE_PAGE_EEPROM
     if (do_page_write) {
	addr_wk = addr_save  & ~mask;
        /* If this device has a "load extended address" command, issue it. */
	if (m->op[AVR_OP_LOAD_EXT_ADDR]) {
        ...
        buf[buf_pos++] = JTAG_RECIEVE | use_delay; 
        buf[buf_pos++] = 4; /* bytes */
        buf[buf_pos++] = 0xc2; /* Issue Page Write */
        buf[buf_pos++] = (addr_wk >> 8) & 0xff;
        buf[buf_pos++] =  addr_wk & 0xff;
        buf[buf_pos++] = 0;
        buf[buf_pos++] = 'r';                /* Request Recieved Data */
        buf[buf_pos++] = 0;
        buf[buf_pos++] = 0;
...
#ifdef USE_WRITE_PAGE_EEPROM
  else if ((strcmp(m->desc, "eeprom") == 0) && (m->paged)) {
    return serjtag_paged_write_eeprom(pgm, p, m, page_size, n_bytes);
  }
#endif
...
static int serjtag_paged_load_eeprom(PROGRAMMER * pgm, AVRPART * p, AVRMEM * m, 
                             int page_size, int n_bytes)
{
  unsigned long    i,j,n;
  //int rc;
  ...
  for (i=0; i<n_bytes; ) {
	...
	if (i >= n_bytes) break;
        buf[buf_pos++] = 0xa0;
        buf[buf_pos++] = (addr >> 8) & 0xff;
        buf[buf_pos++] = (addr) & 0xff;
        buf[buf_pos++] = 0;
	addr ++;
	i++;
     }
                             ...
                             int page_size, int n_bytes)
{
  if (strcmp(m->desc, "flash") == 0) {
    ...
    return serjtag_paged_load_eeprom(pgm, p, m, page_size, n_bytes);
  }
  else {
    return -2;
  }
...
void serjtag_initpgm(PROGRAMMER * pgm)
{
  ...
  pgm->set_sck_period = serjtag_set_delay;
